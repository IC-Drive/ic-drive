...
let keepAlive = () => {
  keepAlive = () => {};
  const ping = `${window.location.href.substr(0, window.location.href.lastIndexOf('/'))}/ping`;
...
export function registerWorker() {
  return navigator.serviceWorker.getRegistration('./').then((swReg) => swReg || navigator.serviceWorker.register('sw.js', { scope: './' })).then((swReg) => {
...
function onMessage(event) {
  ...
  data.streamSaverVersion = new URLSearchParams(window.location.search).get('version');
